...
BRIDGE_URL = os.environ.get("ZYRA_BRIDGE_URL", "https://zyra-bridge.onrender.com")
...
[STEP_COMPLETE]"""
    ...
    for cycle in range(max_swarm_cycles):
        ...
        print(f"\033[95m[Planner -> Coder]\033[0m \033[96m{delegate_instruction}\033[0m\n")
        ...
        for step in range(coder_steps):
            ...
            if cmd_matches:
                ...
                for command in cmd_matches:
                    ...
                    cmd_preview = command if len(command) < 80 else command[:80] + "..."
                    print(f"\033[93m[Coder Action]\033[0m Wants to execute: \033[96m{cmd_preview}\033[0m")
                    ...
                    if auto_yes and not is_dangerous:
                        print(f"\033[93m[Security]\033[0m Auto-approved.")
                    else:
                    ...
                    try:
                        ...
                        out_preview = output_msg.replace('\n', ' ')
                        out_preview = out_preview if len(out_preview) < 100 else out_preview[:100] + "..."
